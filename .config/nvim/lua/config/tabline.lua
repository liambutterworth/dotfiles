...
function GetTabLine()
    ...
    for _, tab in ipairs(tabs) do
        ...
        local icon = string.find(filename, 'term://') ~= nil and '' or ''
        local state = tab == current_tab and 'Active' or 'Inactive'

        if vim.bo[buffer].readonly then
            table.insert(tabline, '%#TabLineReadOnly#')
        elseif vim.bo[buffer].modified then
            table.insert(tabline, '%#TabLineModified#')
        else
            table.insert(tabline, '%#TabLineUnmodified#')
        end

        table.insert(tabline, icon .. '%#TabLine' .. state .. '#')
